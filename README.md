...
| 送迎完了条件 | 実装済み | 全員降車、最後尾QR確認、第三者確認がそろうまで完了不可 |
...
| 動画証跡 | 一部実装 | 動画アップロードと記録保存。30秒の強制撮影は未完成 |
| AI動画チェック | 土台のみ | 判定要求の状態を記録。実際のAIプロバイダーは未接続 |
...
5. 最後尾QR確認後に第三者確認を行い、すべての条件がそろうと送迎を完了できます。

`ACTIVE` は「安全確認が完了した」という意味ではなく、「完了前チェックを開始できる」という意味です。未降車の園児がいる場合、最後尾QR確認がない場合、または第三者確認がない場合は、送迎を完了できません。
...
- 30秒の連続撮影を強制するカメラ画面は未完成
- AIプロバイダー未接続のため、実際の子ども検出・動画判定は未実施
- LINE通知の職員向け画面、再送、到達確認、同意・宛先管理の運用は未完成
- 動画はローカルファイル保存。本番用の暗号化オブジェクトストレージ、保存期限、削除手順、アクセス記録が必要
