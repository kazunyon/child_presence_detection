...
function Operation({trip,routes,vehicles,children,mode,onRoster,onStart,onLeave,onCancel,onScan,onManual,onTail,locationStatus,onApprove,onComplete}:{trip:TripStatus|null;routes:Route[];vehicles:Vehicle[];children:RosterChild[];mode:Mode;onRoster:(ids:number[])=>void;onStart:(r:Route)=>void;onLeave:()=>void;onCancel:()=>Promise<boolean>;onScan:()=>void;onManual:(childId:number)=>void;onTail:()=>void;locationStatus:string;onApprove:(id:number,pin:string)=>void;onComplete:()=>void}) {
