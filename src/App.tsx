...
function RouteRosterEditor({route,children,onEdit,onSave}:{route:Route;children:RosterChild[];onEdit:()=>void;onSave:(ids:number[])=>void}) {
  ...
  return <section className="border-t pt-3 mt-3 text-sm"><div className="flex justify-between"><b>{route.name}・{route.direction}</b><span className="text-slate-500">通常 {selected.length}人</span></div><button className="mt-1 border-0 bg-white p-0 text-xs font-bold text-teal" onClick={onEdit}>便名を変更</button><p className="m-0 mt-1 text-xs text-slate-600">このバスに通常乗る園児を選びます。</p><div className="mt-2 grid grid-cols-2 gap-2">{children.map(child=><label key={child.id} className="flex items-center gap-2 rounded-lg bg-slate-50 p-2"><input type="checkbox" checked={selected.includes(child.id)} onChange={()=>toggle(child.id)}/><span>{child.name}</span></label>)}</div><button className="mt-2 w-full rounded-xl bg-slate-800 p-2 font-bold text-white" onClick={()=>onSave(selected)}>通常名簿を保存</button></section>
}
