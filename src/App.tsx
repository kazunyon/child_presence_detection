...
type LineLinkPreview = { request_id:number; status:string; expires_at:string; email_delivery_status:string; official_account_name:string; line_basic_id:string; line_link_url:string; qr_png_data_url:string }
...
function Operation({trip,routes,vehicles,children,mode,onRoster,onStart,onLeave,onCancel,onScan,onManual,onTail,onVideo,locationStatus,onApprove,onComplete}:{trip:TripStatus|null;routes:Route[];vehicles:Vehicle[];children:RosterChild[];mode:Mode;onRoster:(ids:number[])=>void;onStart:(r:Route)=>void;onLeave:()=>void;onCancel:()=>Promise<boolean>;onScan:()=>void;onManual:(childId:number)=>void;onTail:()=>void;onVideo:()=>void;locationStatus:string;onApprove:(id:number,pin:string)=>void;onComplete:()=>void}) {
  ...
  const hasVideo = (trip.video_evidence_count || 0) > 0
  const videoStatus = trip.latest_video_ai_result || (hasVideo ? '動画を保存しました。職員が車内を再確認してください。' : '')
...
function Settings({operator,auth,onMessage,onRefresh}:{operator:Operator;auth:AuthBuilder;onMessage:(message:string)=>void;onRefresh:()=>Promise<void>}) {
  const [organization,setOrganization]=useState<OrganizationInfo|null>(null),[children,setChildren]=useState<SettingChild[]>([]),[staff,setStaff]=useState<SettingStaff[]>([]),[vehicles,setVehicles]=useState<Vehicle[]>([]),[routes,setRoutes]=useState<Route[]>([]),[guardians,setGuardians]=useState<GuardianContact[]>([]),[notifications,setNotifications]=useState<NotificationItem[]>([]),[loading,setLoading]=useState(true)
  ...
  const load=async()=>{
    ...
    await loadOne<GuardianContact[]>('保護者通知先','/api/guardian-contacts',setGuardians)
    await loadOne<NotificationItem[]>('通知履歴','/api/notifications',setNotifications)
...
function GuardianNotificationSettings({children,guardians,notifications,preview,onCreate,onUpdate,onIssue,onUnlink,onRetry,onClosePreview}:{children:SettingChild[];guardians:GuardianContact[];notifications:NotificationItem[];preview:LineLinkPreview|null;onCreate:(data:Record<string,unknown>)=>Promise<void>;onUpdate:(id:number,data:Record<string,unknown>)=>Promise<void>;onIssue:(item:GuardianContact)=>Promise<void>;onUnlink:(item:GuardianContact)=>Promise<void>;onRetry:(item:NotificationItem)=>Promise<void>;onClosePreview:()=>void}) {
  ...
  const edit=async(item:GuardianContact)=>{
    const name=window.prompt('保護者名',item.name||'')
    if(name===null)return
    const email=window.prompt('メールアドレス',item.email)
    if(!email)return
    await onUpdate(item.id,{name:name||null,email})
  }
  ...
  return <>
    <section className="card mt-4 p-4">
      ...
      <div className="mt-4 divide-y">{guardians.length===0?<p className="text-sm text-slate-600">登録済みの保護者通知先はありません。</p>:guardians.map(item=><article className="py-4" key={item.id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b>{item.name||'保護者'}</b><p className="m-0 mt-1 break-all text-sm text-slate-600">{item.email}</p><p className="m-0 mt-1 text-xs text-slate-500">{item.children.map(child=>child.name).join('・')||'園児未設定'}／メール {item.email_enabled?'ON':'OFF'}／LINE {item.line_enabled?'ON':'OFF'}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-xs font-bold ${statusClass(item.line_status)}`}>{statusLabel(item.line_status)}</span></div><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-lg border bg-white px-3 py-2 text-xs font-bold" onClick={()=>void edit(item)}>編集</button>{item.consented_at&&!item.line_enabled&&<button className="rounded-lg bg-teal px-3 py-2 text-xs font-bold text-white" onClick={()=>void onUpdate(item.id,{email_enabled:true,line_enabled:true})}>LINE通知を有効化</button>}{item.line_enabled&&<button className="rounded-lg bg-teal px-3 py-2 text-xs font-bold text-white" onClick={()=>void onIssue(item)}>{item.line_status==='pending'||item.line_status==='expired'?'QRを再発行':'QR案内を発行'}</button>}{item.line_enabled&&item.line_status!=='linked'&&<button className="rounded-lg border bg-white px-3 py-2 text-xs font-bold" onClick={()=>void onUpdate(item.id,{email_enabled:true,line_enabled:false})}>LINE希望を停止</button>}{item.line_status==='linked'&&<button className="rounded-lg border border-red-200 bg-white px-3 py-2 text-xs font-bold text-red-700" onClick={()=>void onUnlink(item)}>LINE連携解除</button>}{item.consented_at?<button className="rounded-lg border bg-white px-3 py-2 text-xs font-bold text-slate-600" onClick={()=>window.confirm(`${item.name||item.email} の通知同意を撤回し、LINE・メール通知を停止しますか？`)&&void onUpdate(item.id,{consent:false})}>通知停止・同意撤回</button>:<button className="rounded-lg border bg-white px-3 py-2 text-xs font-bold text-teal" onClick={()=>void onUpdate(item.id,{consent:true,email_enabled:true,line_enabled:false})}>メール通知を再開</button>}</div></article>)}</div>
    </section>
    {preview&&<section className="card mt-4 p-4"><div className="flex justify-between gap-3"><div><h2 className="m-0 text-lg font-black">QR連携案内</h2><p className="m-0 mt-1 text-xs text-slate-600">{preview.official_account_name}（{preview.line_basic_id}）／メール：{preview.email_delivery_status}</p></div><button className="border-0 bg-white text-sm font-bold" onClick={onClosePreview}>閉じる</button></div><div className="mt-4 grid place-items-center"><img className="w-52 max-w-full rounded-xl border bg-white p-2" src={preview.qr_png_data_url} alt={`${preview.official_account_name} LINE連携用QRコード`}/></div><a className="mt-3 block break-all rounded-xl bg-slate-50 p-3 text-sm font-bold text-teal" href={preview.line_link_url} target="_blank" rel="noreferrer">同じ端末でLINE連携を開く</a><p className="mb-0 mt-2 text-xs text-slate-600">有効期限：{formatDateTime(preview.expires_at)}。この画面を閉じると同じリンクは再表示できません。必要時は再発行してください。</p></section>}
