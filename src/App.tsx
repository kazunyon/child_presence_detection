...
export default function App() {
  ...
  const content = view==='home' ? <Home dashboard={dashboard} onOperation={openOperation}/> : view==='operation' ? <Operation trip={trip} routes={routes} vehicles={vehicles} children={children} mode={mode} onRoster={updateRoster} onStart={startTrip} onLeave={leaveOperation} onCancel={cancelTripForReselection} onScan={()=>setScanner('child')} onManual={manualAttendance} onTail={()=>setScanner('tail')} locationStatus={locationStatus} onApprove={approve} onComplete={complete}/> : view==='children' ? <ChildrenPage operator={operator} auth={auth} onMessage={setMessage} onRefresh={loadBootstrap}/> : view==='records' ? <Records operator={operator} auth={auth} onMessage={setMessage} onResume={resumeTrip}/> : <Settings operator={operator} auth={auth} onMessage={setMessage} onRefresh={loadBootstrap}/>
