...
export default function App() {
  ...
  const returnSafetyCheck = async () => {
    if (!trip || trip.tail_confirmed) return
    setLocationStatus('車内確認を記録しています…')
    ...
    try {
      const r = await fetch(`${API}/api/vehicle-checks`,auth({method:'POST',body:JSON.stringify({trip_id:trip.trip_id,check_type:'tail_qr',qr_token:'return-vehicle-check',latitude,longitude})}))
      if (!r.ok) throw new Error(await messageOf(r))
      await refresh(trip.trip_id)
      setLocationStatus(latitude&&longitude?'位置情報を付けて車内確認を記録しました':'車内確認を記録しました')
      setMessage('車内確認を記録しました。第三者確認へ進んでください。')
    } catch (error) { setLocationStatus('車内確認を保存できませんでした'); setMessage(error instanceof Error ? error.message : '車内確認を保存できませんでした') }
  }
