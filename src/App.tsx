...
export default function App() {
  ...
  const cancelTripForReselection = async () => {
    if (!trip) return
    if (!window.confirm('この送迎を中止して、バス選択へ戻ります。QRなしを含む乗降・安全確認をまだ記録していない場合だけ中止できます。')) return
    try {
    ...
    } catch(error) { setMessage(error instanceof Error ? error.message : '送迎を中止できませんでした') }
  }
...
function Operation({trip,routes,vehicles,children,mode,onRoster,onStart,onLeave,onCancel,onScan,onManual,onTail,locationStatus,onApprove,onComplete}:{trip:TripStatus|null;routes:Route[];vehicles:Vehicle[];children:RosterChild[];mode:Mode;onRoster:(ids:number[])=>void;onStart:(r:Route)=>void;onLeave:()=>void;onCancel:()=>void;onScan:()=>void;onManual:(childId:number)=>void;onTail:()=>void;locationStatus:string;onApprove:(id:number,pin:string)=>void;onComplete:()=>void}) {
...
const formatDateTime = (value:string|null|undefined) => value ? new Date(value).toLocaleString('ja-JP',{timeZone:'Asia/Tokyo',timeZoneName:'short',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}) : '—'
