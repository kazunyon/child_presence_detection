...
export default function App() {
  ...
  return <div className="app"><header className="px-5 pt-5 pb-4 bg-white flex justify-between"><div><div className="text-xs font-bold text-teal">送迎バス安全確認</div><b className="text-xl">まもるバス</b></div><button className="border-0 bg-white text-sm font-bold" onClick={logout}>{operator.name}</button></header><main className="px-5 pb-24">{offlineCount>0&&<button className="w-full rounded-xl bg-amber-100 p-3 text-sm font-bold text-amber-900" onClick={sync}>未同期の記録 {offlineCount} 件 — 同期する</button>}<section className="mt-3 rounded-2xl bg-sand p-4 border border-amber-100"><p className="m-0 text-sm">{message}</p></section>{content}</main><Nav active={view} onChange={setView}/> {scanner&&<Scanner title={scanner==='child'?`${mode}QRを読み取る`:'最後尾QRを読み取る'} onRead={scanner==='child'?scanChild:tail} onClose={()=>setScanner(null)}/>}</div>
}
